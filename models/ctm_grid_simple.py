...
class ContinuousThoughtMachineGridDiff(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 multi_input_attention = 0
                 ):
        super(ContinuousThoughtMachineGridDiff, self).__init__()
        ...
        self.multi_input_attentions = [nn.MultiheadAttention(self.d_input, heads, dropout, batch_first=True) for _ in range(multi_input_attention)] if multi_input_attention > 0 else None
    ...
    def forward(self, x, track=False):
        ...
        # M is the number of different inputs (number of different grids etc)
        B, M = x.size(0), x.size(1)
        
        assert M == len(self.multi_input_attentions) if self.multi_input_attentions is not None else 1, \
            f"Custom Error: Input x has {M} slices, but the model expects {len(self.multi_input_attentions) if self.multi_input_attentions is not None else 1} slices."
        ...
        kv_list = []
        for m in range(M):
            # take the m’th input slice: (B, …)
            x_m = x[:, m, ...]
            
            # compute_features returns (k, v) of shape (B, T, d_input)
            kv_m = self.compute_features(x_m)
            kv_list.append(kv_m)
        ...
        for stepi in range(self.iterations):
            ...
            attn_outs, attn_weights_all = [], []
            for m in range(M):
                head = self.multi_input_attentions[m]
                # head expects: query=(B,1,d_input), key/value=(B,T,d_input)
                out_m, weights_m = head(q,
                                    kv_list[m],
                                    kv_list[m],
                                    average_attn_weights=False,
                                    need_weights=True)
                attn_outs.append(out_m.squeeze(1))     # (B, d_input)
                attn_weights_all.append(weights_m)     # (B,1,T)
            # now merge M outputs along feature‐dim:
            attn_out     = torch.cat(attn_outs,      dim=-1)   # (B, M*d_input)
            attn_weights = torch.stack(attn_weights_all, dim=1) # (B, M, 1, T)
