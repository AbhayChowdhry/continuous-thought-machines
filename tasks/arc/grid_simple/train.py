...
def parse_args():
    ...
    parser.add_argument('--iterations', type=int, default=5, help='Number of internal ticks (CTM, LSTM).')
    ...
    parser.add_argument('--log_dir', type=str, default='logs/multi_attn_diff_1000_samples', help='Directory for logging.')
    parser.add_argument('--multi_input_attention', type=int, default=2, help='Number of inputs (grids) to process')
    
    args = parser.parse_args()
    return args

# def vizualise_matrix(matrix):
...
def get_dataset(dataset, root):
    ...
    if dataset=='grid_dif':
        path_digit = r"C:\Users\abhay\OneDrive\IIITD\ARC\continuous-thought-machines\data\grid_diff\grid_diff_dataset.json"
        # path_digit = r"C:\Users\abhay\OneDrive\IIITD\ARC\continuous-thought-machines\data\grid_num_class\grid_digit_label_dataset.json"
        train_data, test_data = load_and_split_dataset(path_digit, test_size=0.2)
        class_labels = [0, 1]  
        # class_labels = [i for i in range(10)]  

        dataset_mean = [0.5, 0.5, 0.5]  # Dummy values, not used
        dataset_std = [0.5, 0.5, 0.5]  # Dummy values, not used
...
if __name__=='__main__':
    ...
    if args.model == 'ctm':
        model = ContinuousThoughtMachineGridDiff(
            ...
            multi_input_attention=args.multi_input_attention 
        ).to(device)
